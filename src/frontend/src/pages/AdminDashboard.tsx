...
function ModalShell({ title, onClose, children }: any) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, zIndex: 100, backgroundColor: "rgba(0,0,0,0.8)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
...
function AddLectureModal({ onClose, onSuccess, actor }: any) {
  ...
  const handleSave = async () => {
    if (!actor) return alert("Connection not ready.");
    setLoading(true);
    try {
  ...
  return (
    <ModalShell title="Add Lecture" onClose={onClose}>
      ...
      <button onClick={handleSave} disabled={loading} style={{ width: "100%", background: "#8C3A3A", color: "#fff", padding: "1rem", border: "none", marginTop: "1rem" }}>{loading ? "SAVING..." : "SAVE LECTURE"}</button>
    </ModalShell>
  );
}
...
function AddStudentWorkModal({ onClose, onSuccess, actor }: any) {
  ...
  return (
    <ModalShell title="Add Student Work" onClose={onClose}>
      ...
      <label style={labelStyle}>Description</label>
      <textarea style={{ ...inputStyle, height: "80px" }} value={desc} onChange={(e) => setDesc(e.target.value)} />
      <input type="file" onChange={handleFile} style={{ marginTop: "1rem" }} />
      <button onClick={handleSave} disabled={loading} style={{ width: "100%", background: "#8C3A3A", color: "#fff", padding: "1rem", border: "none", marginTop: "1rem" }}>{loading ? "SAVING..." : "SAVE WORK"}</button>
    </ModalShell>
  );
}
...
function AddArtItemModal({ onClose, onSuccess, actor }: any) {
  ...
  return (
    <ModalShell title="Add Art Item" onClose={onClose}>
      ...
      <input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} />
      <input type="file" onChange={handleFile} style={{ marginTop: "1rem" }} />
      {img && <img src={img} style={{ width: "100%", height: "120px", objectFit: "cover", marginTop: "1rem" }} />}
      <button onClick={handleSave} disabled={loading} style={{ width: "100%", background: "#8C3A3A", color: "#fff", padding: "1rem", border: "none", marginTop: "1rem" }}>{loading ? "SAVING..." : "SAVE ART"}</button>
    </ModalShell>
  );
}
...
function AddResearchItemModal({ onClose, onSuccess, actor }: any) {
  ...
  const handleSave = async () => {
    if (!actor) return alert("Cannot read properties of null (waiting for actor connection)");
    setLoading(true);
    try {
  ...
  return (
    <ModalShell title="Add Research Card" onClose={onClose}>
      ...
      <input type="file" onChange={handleFile} style={{ marginTop: "1rem" }} />
      <button onClick={handleSave} disabled={loading} style={{ width: "100%", background: "#8C3A3A", color: "#fff", padding: "1rem", border: "none", marginTop: "1rem" }}>{loading ? "SAVING..." : "SAVE RESEARCH"}</button>
    </ModalShell>
  );
}
...
export function AdminDashboard() {
  ...
  const loadData = useCallback(async (section: NavSection) => {
    ...
    try {
      ...
      setDataList(data);
    } catch (e: any) { setLoadError(e.message); } finally { setIsLoading(false); }
  }, [actor]);
    ...
    <div style={{ display: "flex", height: "100vh", background: "#000", color: "#E5E0D8", fontFamily: "monospace" }}>
      <aside style={{ width: "240px", background: "#111", borderRight: "1px solid #222", display: "flex", flexDirection: "column" }}>
        ...
        <nav style={{ flex: 1 }}>
          {[
            ...
            <button key={item.id} onClick={() => setActiveSection(item.id as NavSection)} style={{ width: "100%", display: "flex", alignItems: "center", gap: "1rem", padding: "0.8rem 1.5rem", background: "transparent", border: "none", borderLeft: activeSection === item.id ? "3px solid #8C3A3A" : "3px solid transparent", opacity: activeSection === item.id ? 1 : 0.4, cursor: "pointer", color: "#E5E0D8", textAlign: "left" }}>
              {item.icon} <span style={{ fontSize: "10px", textTransform: "uppercase" }}>{item.label}</span>
            </button>
          ))}
        </nav>
        <div style={{ padding: "1.5rem" }}>
          <button onClick={() => iiLogin()} style={{ width: "100%", background: "#8C3A3A", color: "#fff", border: "none", padding: "0.6rem", fontSize: "10px", marginBottom: "1rem" }}>{identity ? "RE-AUTHENTICATE" : "CONNECT IDENTITY"}</button>
          <button onClick={() => { logout(); navigate({ to: "/admin" }); }} style={{ width: "100%", background: "none", color: "gray", border: "none", fontSize: "10px" }}>SIGN OUT</button>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, padding: "4rem", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3rem" }}>
          <div>
            <h1 style={{ fontFamily: "serif", fontStyle: "italic", fontSize: "32px", margin: 0 }}>{SECTION_META[activeSection].title}</h1>
            <p style={{ fontSize: "11px", color: "gray" }}>{SECTION_META[activeSection].description}</p>
          </div>
          <button onClick={() => setShowAddModal(true)} style={{ background: "#8C3A3A", color: "#fff", border: "none", padding: "0.8rem 2rem", fontSize: "11px" }}>+ ADD NEW</button>
        </div>

        {loadError && <div style={{ color: "#8C3A3A", marginBottom: "2rem" }}>⚠ {loadError}</div>}

        <div style={{ background: "#0a0a0a", border: "1px solid #1a1a1a" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px", padding: "1rem", background: "#111", fontSize: "9px", color: "gray" }}>
          ...
          {isLoading ? <p style={{ padding: "2rem", textAlign: "center" }}>Loading...</p> : 
            dataList.map((item, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px", padding: "1rem", borderBottom: "1px solid #111", fontSize: "12px" }}>
                <span>{item.title || item.studentName}</span>
                <span style={{ fontSize: "9px", color: item.isLive ? "#4CAF50" : "#444" }}>{item.isLive ? "LIVE" : "DRAFT"}</span>
                <button style={{ background: "none", border: "1px solid #333", color: "gray", fontSize: "9px" }}>MANAGE</button>
              </div>
            ))
          }
        </div>
