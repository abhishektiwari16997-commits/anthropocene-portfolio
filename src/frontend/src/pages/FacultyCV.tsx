...
// THE ABSOLUTE FINAL PATH: Matches frontend/public/assets/uploads/cv.pdf
...
export function FacultyCV() {
  ...
  useEffect(() => {
    ...
    actor
      .getCvPdf()
      .then((data) => {
        // Use backend data if available, otherwise use our verified static file
  ...
  // Fallback timeout: ensures the PDF shows even if the blockchain is slow
  useEffect(() => {
    const t = setTimeout(() => {
      if (loading) {
        setPdfSrc(STATIC_CV);
