...
export function FacultyCV() {
  ...
  useEffect(() => {
    if (!actor) return;
    actor.getCvPdf()
      .then((data) => setPdfSrc(data?.trim() ? data : STATIC_CV))
  ...
  return (
    <div style={{ position: "relative", width: "100%", minHeight: "100dvh", backgroundColor: "#000000", display: "flex", flexDirection: "column", cursor: "none" }}>
      <div aria-hidden="true" style={{ position: "fixed", inset: 0, zIndex: 10, pointerEvents: "none", backgroundImage: GRAIN_SVG, opacity: 0.038 }} />
      <AnthropoceneAnchor />
      <FacultySubNav />
      {loading ? (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <p style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: "10px", color: "#8C3A3A" }}>LOADING CV...</p>
        </div>
      ) : (
        <embed src={pdfSrc ?? STATIC_CV} type="application/pdf" style={{ flex: 1, width: "100%", height: "calc(100dvh - 120px)", border: "none", zIndex: 5 }} />
      )}
    </div>
  );
}
